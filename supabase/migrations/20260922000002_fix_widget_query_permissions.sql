-- Fix: run_widget_query, can_read_board and the members_read policy referenced
-- auth.users, which regular users cannot read -> "permission denied for table users".
-- Read the caller's email from their JWT instead (always available, no special rights).
      ...
      and (d.owner_id = auth.uid()
        or exists (
          ...
          where m.board_id = p_board
            and m.member_email = lower(coalesce(auth.jwt() ->> 'email', ''))
        ))
  );
$$;
...
create policy members_read on public.board_members
  for select using (
    invited_by = auth.uid()
    or member_email = lower(coalesce(auth.jwt() ->> 'email', ''))
  );
